pipeline {
  ...
  environment {
    BOT_NAME = 'llm_harness_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-auditor_llm_harness"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/llm_harness_bot/bin/llm_harness_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorLlmHarness.Release.create()'"
          $RELEASE_BIN eval 'BotArmyAuditorLlmHarness.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorLlmHarness.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyAuditorLlmHarness.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_auditor_llm_harness/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_auditor_llm_harness/releases/start_erl.data)
        fi
